...
import { Sku, SkuClass, SkuModelView } from '@/entity/sku'
import { defaultSkuModelView, editSku } from '@/api/skus'
...
export default class extends Vue {
  ...
  @Prop({ type: Object }) private readonly sku!: Sku
  ...
  private open() {
    this.form = {
      brandName: this.sku.brandName,
      classId: this.sku.classId,
      price: this.sku.price / 100,
      skuImage: this.sku.skuImage,
  ...
  private async ensureDialog() {
    try {
      ...
      // 提交表单
      await editSku(this.sku.skuId, data)
