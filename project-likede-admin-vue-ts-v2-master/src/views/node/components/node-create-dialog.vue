...
import { defaultNodeModelView, createNode } from '@/api/nodes'
import { BusinessType } from '@/entity/vm'
import { NodeModelView } from '@/entity/node'
...
export default class extends mixins(MixinTools) {
  ...
  // 详细地址
  private showAreaCascader = false // 强制area-cascader重新渲染
  private pcaa: any = pcaa
  private areaCascaderSel: String[] = []
  ...
  private async ensureDialog() {
    try {
      ...
      let node: NodeModelView = { ...this.form }
      ...
      node.areaCode = Number(cityCode)
      node.addr = this.areaCascaderSel.join('-') + '-' + node.addr
      // 提交表单
