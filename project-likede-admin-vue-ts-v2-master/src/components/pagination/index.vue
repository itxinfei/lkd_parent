<template>
  ...
  >
    ...
    >
      <span class="el-pagination__total">共{{ total }}条记录&nbsp;&nbsp;第&nbsp;{{ currentPage }}/{{ Math.ceil(total / pageSize) }}&nbsp;页</span>
    </el-pagination>
  </div>
</template>
...
export default class extends Vue {
  ...
  @Prop({ default: 'slot, prev, next' }) private layout!: string
  @Prop({ default: true }) private background!: boolean
  @Prop({ default: true }) private autoScroll!: boolean
  @Prop({ default: false }) private hidden!: boolean
  @Prop({ default: '上一页' }) private prevText!: string
  @Prop({ default: '下一页' }) private nextText!: string
